...
kable(diagnosis %>% head(n = 15), "html") %>%
  kable_styling(full_width = F, position = "float_left") #add , font-size = 2 to change the fontsize of the table

#clean the data, remove unnesecary collums like hbt, ive set it so its only the first 15 rows.
```
---
```{r dataset2, echo=FALSE}
kable(deprivation  %>% head(n = 15), "html") %>%
